...
#include <afxconv.h>           // For LPTSTR -> LPTSTR macros
...
#pragma message(_T(" -- CGridCtrl: No titletips for cells with large data"))
#endif
#ifdef GRIDCONTROL_NO_DRAGDROP
#pragma message(_T(" -- CGridCtrl: No OLE drag and drop"))
#endif
#ifdef GRIDCONTROL_NO_CLIPBOARD
#pragma message(_T(" -- CGridCtrl: No clipboard support"))
#endif
#ifdef GRIDCONTROL_NO_PRINTING
#pragma message(_T(" -- CGridCtrl: No printing support"))
#endif
...
LRESULT CGridCtrl::SendMessageToParent(int nRow, int nCol, int nMessage, DWORD dwParam /*= 0*/) const
{
    if (!IsWindow(m_hWnd))
        return 0;
...
{
	//GVN_ITEMKEYDOWN
	if (SendMessageToParent(m_idCurrentCell.row, m_idCurrentCell.col, GVN_ITEMKEYDOWN, nChar) >= 0)
	{
		...
		if (IsCTRLpressed())
		{
			...
			case _T('A'):
				OnEditSelectAll();
				break;
			case _T('k'):		// This is ctrl+ on french keyboard, may need to be better processed for other locales
				AutoSizeColumns();
				Invalidate();
				break;
#ifndef GRIDCONTROL_NO_CLIPBOARD
...
{
	if (SendMessageToParent(m_idCurrentCell.row, m_idCurrentCell.col, GVN_ITEMKEYUP, nChar) >= 0)
	{
...
{
	// VRI - It's not a bug to me, that's normal  to not directly edit cell if not
	// in edit mode.
	if (m_bEditModeOnKeyDown == TRUE)
	{
...
void CGridCtrl::SetSelectedRows(int nMinRow, int nMaxRow)
{
	SetSelectedRange(nMinRow, 0, nMaxRow, GetColumnCount() - 1, FALSE, TRUE);
}
...
COleDataSource* CGridCtrl::CopyTextFromGrid()
{
    ...
    // Write to shared file (REMEMBER: CF_TEXT is ANSI, not UNICODE)
								...
								BOOL bSelectPastedCells /*=TRUE*/)
{
	...
	{
		hmem = pDataObject->GetGlobalData(CF_UNICODETEXT);
		strText = (LPCTSTR)::GlobalLock(hmem);
	}
	else
#endif
...
BOOL CGridCtrl::DeleteAllItems(BOOL bDeleteColumns /*= TRUE*/)
{
    ResetSelectedRange();

	if (bDeleteColumns == TRUE)
    ...
    m_idCurrentCell.row = m_idCurrentCell.col = -1;
    m_nRows = m_nFixedRows = 0;
	if (bDeleteColumns == TRUE)
		m_nCols = m_nFixedCols = 0;
...
void CGridCtrl::SetVirtualMode(BOOL bVirtual, BOOL bDeleteColumns /*= TRUE*/)
{
    DeleteAllItems(bDeleteColumns);
    m_bVirtualMode = bVirtual;
	m_bVirtualDelColumns = bDeleteColumns;
...
void CGridCtrl::SetAutoSizeStyle(int nStyle /*= GVS_BOTH*/, BOOL bFastMethod /*= FALSE*/)
{
	m_nAutoSizeColumnStyle = nStyle; 
	m_bAutoSizeColumnFast = bFastMethod;
}
...
{
    TRACE(_T("CGridCtrl::OnLButtonDblClk\n"));
...
#endif
    {
        ...
        Invalidate();
		SendMessageToParent(0, cell.col, GVN_ENDTRACK);
    }
#ifdef _WIN32_WCE
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    // TRACE(_T("CGridCtrl::OnLButtonDown\n"));
    ...
    if (m_MouseMode == MOUSE_OVER_COL_DIVIDE) // sizing column
    {
        ...
        ClipCursor(rect);
#endif
		SendMessageToParent(0, m_LeftClickDownCell.col, GVN_BEGINTRACK);
    }
...
{
    // TRACE(_T("CGridCtrl::OnLButtonUp\n"));
    ...
    else if (m_MouseMode == MOUSE_SIZING_COL)
    {
        ...
        {   
			...
			SendMessageToParent(0, m_LeftClickDownCell.col, GVN_ENDTRACK);
        }
    }
