                     ...
                     COLORREF crBackClr /* CLR_DEFAULT */)
{
	...
	// DT_CALCRET offer more options compared to GetTextExtent and will allow to implement word break
	//CSize size = dc.GetTextExtent( strTitle );
	
	int nDrawTextFormat = DT_SINGLELINE;
	CRect rcText(0,0,0,0);
	dc.DrawText(strTitle, &rcText, nDrawTextFormat | DT_CALCRECT);
	CSize size = rcText.Size();
    ...
    if ( rectDisplay.right > rectTitle.right-xoffset )
    {
		...
		dc.DrawText(strTitle, &rcText, nDrawTextFormat);
